...
from Similarity_Analyzer import jaccard_similarity, preprocess_text
...
class MultiThreadCrawler:
    ...
    def _check_duplicates(self, content:str):
        sep_words = preprocess_text(content)

        # 若存在相似度大于 0.85 则不保存
        for words in self._sep_words_list:
            sim = jaccard_similarity(sep_words, words)
            if sim > SimilarityThreshold:
                logger.info(f"相似度 = {sim} 过高，丢弃")
                return False

        self._sep_words_list.append(sep_words)
        return True
    ...
    def handle_task(self):
        ...
        while True:
            ...
            if not self._check_duplicates(item.Content):
                continue
